...
public class QuickSortV3 implements SortInterface {
    ...
    public void sort(Comparable[] F, StatObject so) { // overload method sort for testing
        this.so = so;
        sort(F, 0, F.length - 1, 1); // call the normal sort method this sort(int[] F, int u, int o)
    ...
    public void sort(Comparable[] F, int u, int o, int distance) {
        ...
        if (o > u) { // runs as long as the upper cabinets is greater than the under
            ...
            so.incrunCounter();
            int i = split(F, u, o, distance);// call split method and write the index to i
            sort(F, u, i - 1, distance); // set upper cabinets one down
            sort(F, i + 1, o, distance); // set under cabinets one up
        }
    ...
    public int split(Comparable[] F, int u, int o, int distance) {
        ...
        for (int zeiger = u; zeiger <= o - 1; zeiger++) {
            ...
            if ((F[zeiger].compareTo(F[p]) * distance) >= 0) {
